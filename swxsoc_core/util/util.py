...
def create_science_filename(
    ...
    if instrument not in swxsoc_core.INST_NAMES:
        raise ValueError(
            f"Instrument, {instrument}, is not recognized. Must be one of {swxsoc_core.INST_NAMES}."
        )
    ...
    filename = f"swxsoc_{swxsoc_core.INST_TO_SHORTNAME[instrument]}_{mode}_{level}{test_str}_{descriptor}_{time_str}_v{version}"
...
def parse_science_filename(filepath: str) -> dict:
    ...
    if filename_components[0] != swxsoc_core.MISSION_NAME:
        raise ValueError(f"File {filename} not recognized. Not a valid mission name.")

    if file_ext == ".bin":
        if filename_components[1] not in swxsoc_core.INST_TARGETNAMES:
            raise ValueError(
        ...
        from_shortname = {v: k for k, v in swxsoc_core.INST_TO_TARGETNAME.items()}

        result["time"] = Time.strptime(filename_components[3 + offset], TIME_FORMAT_L0)

    elif file_ext == ".cdf":
        if filename_components[1] not in swxsoc_core.INST_SHORTNAMES:
            raise ValueError(
                "File {filename} not recognized. Not a valid instrument name."
            )

        #  reverse the dictionary to look up instrument name from the short name
        from_shortname = {v: k for k, v in swxsoc_core.INST_TO_SHORTNAME.items()}
